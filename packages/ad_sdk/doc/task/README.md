...
- **Vòng verify thứ 2 (cùng ngày):** 5 ticket còn "chưa verify độc lập" (T62, T63, T65, T66, T67) đã được codex/agy/claude CLI verify hết — không cái nào bị refute, tất cả CONFIRMED hoặc PLAUSIBLE (thu hẹp phạm vi). Toàn bộ **43 ticket T57-T99 giờ đều đã qua ít nhất 1 vòng verify độc lập**, sẵn sàng vào sprint code.
- **Bắt đầu fix theo TDD:** **T57 hoá ra REFUTED** — viết test trước (TDD) mới lộ ra `RouteObserver.subscribe()` của Flutter SDK luôn tự gọi `didPush()` ngay khi subscribe (không cần route mới toanh), khác với claim "hành vi chuẩn Flutter" mà 2 audit + 1 verify độc lập trước đó đều tin nhầm. T57 → `done/` (REFUTED), T81 (test bổ sung) → `done/` (2 test mới vẫn giữ làm regression). `flutter test`: 702/702 pass.
- **T58 done (2026-08-15):** fix thật — `estimatedEcpmMicros` nhân `* 1000` (per-impression revenue → eCPM per-1000). TDD lộ thêm phát hiện: toàn bộ test suite cũ của arbitrator dùng giá trị synthetic sai thang giống hệt bug trong code, nên chưa từng bắt được — đã sửa 17+1 input test về đúng thang per-impression thật. `flutter test`: 704/704 pass, `flutter analyze` sạch.
- **T59 done (2026-08-15):** fix thật — `setRaw()` và nhánh migration của `getRaw()` trong `_vip_entries_store.dart` giờ chỉ mark migrated khi ghi secure storage thật sự thành công. Debug lộ thêm: `getRaw()` cũng dính đúng lỗi này (không chỉ `setRaw()` như ticket gốc mô tả) — sửa chung 1 lần, cùng root cause. Giới hạn còn lại (Keystore hỏng vĩnh viễn + app bị kill trước khi retry vẫn mất data) thuộc scope **T71** (fallback storage), không mở rộng ticket này. `flutter test`: 706/706 pass, `flutter analyze` sạch. Chi tiết: `doc/task/BACKLOG-sdk-2026-08-15.md`.
- **3/3 bug ưu tiên (T57-T59) đã xử lý xong** theo TDD tuần tự (1 commit/bug, đúng approach đã chọn). T57 refuted, T58+T59 fix thật.
- **T60 done (2026-08-15):** fix thật — `_maybeScheduleConsentDialog` giờ clear `_footgunBlocked` sau khi dialog built-in trả lời (giống `setConsent()` đã làm), thêm test seam `debugConsentManager`. `flutter test`: 707/707 pass.
- **T61 done (2026-08-15):** docs-only — README default `autoRequestUmpConsent` sửa `false` → `true` khớp code.
- **T62 done (2026-08-15) — PHÁT HIỆN NGHIÊM TRỌNG:** claim gốc ("gate không re-check") hoá ra chỉ là triệu chứng của root cause lớn hơn nhiều — `_NativeContainer` gate `child()` (mount `MaxNativeAdView` thật) đằng sau `isLoaded`, nhưng `isLoaded` CHỈ được set bởi chính callback của `MaxNativeAdView`, và `preloadNative()` AppLovin là no-op cố ý → **deadlock thật: native ad AppLovin không bao giờ load được trong production, không phải edge case**. Fix: `_NativeContainer` giờ luôn mount `child()`, shimmer chỉ overlay. TDD viết test KHÔNG tự gán `isLoaded=true` (khác test cũ) mới lộ ra được. `flutter test`: 708/708 pass. Phần "re-check gate khi rebuild" tách thành **T100** (P2, giờ mới có ý nghĩa để test). - **T63 done (2026-08-15):** fix có phạm vi rõ ràng — `_resetGuardState()` giờ reset `_canRequestAds`→`true` và `_umpAttemptFailed`→`false` (2 field có tác động chức năng thật theo verify trước đó); cố ý không đụng `_lastUmpResult`/`_attRequested` (rủi ro thấp, ngoài scope). `flutter test`: 709/709 pass. - **T64 done (2026-08-15):** fix thật — `canShowInterstitial()`/`canShowRewardedAd()` giờ check `canRequestAds` trước khi show, chặn ad đã cache hiển thị sau khi consent bị revoke (vấn đề tuân thủ GDPR, không chỉ UX). Quirk VIP-bypass của rewarded giữ nguyên, không đụng. `flutter test`: 711/711 pass.
- **T65 done (2026-08-16) — refactor lớn nhất round này:** ticket gốc chỉ nêu AdMob banner/mrec/native dùng singleton field trong adapter (2 widget cùng loại mount cùng lúc → crash `FlutterError: 'This AdWidget is already in the Widget tree'`). Scoping phát hiện AppLovin banner/mrec cũng dính lỗi tương tự (native AppLovin thì không). User chọn fix full N-instance thật cho cả 6 tổ hợp (banner/mrec/native × AdMob/AppLovin), theo kiến trúc "keyed-by-instance": mỗi widget `State` tự có 1 `Object` key riêng, adapter lưu `AdSlot`/`BannerListenables`/live-ad-object dưới `Map<Object, T>` thay vì field đơn. 3 phase, 1 commit/phase (`2267801`, `32a03af`, `8c367e9`), phát hiện thêm 2 bug phụ (AppLovin native shimmer-state rò rỉ chéo instance; banner/mrec preload-timing cần sentinel key để không vỡ test reconnect). Phase 4: thêm integration test `example/integration_test/multi_instance_ad_test.dart` + section instance-thứ-2 vào 3 demo page, chạy **3/3 pass** trên Pixel 7 Pro thật (`2B051FDH3006MU`), smoke test tay qua `mcp__mobile__*` xác nhận 2 instance/loại render độc lập không crash (`mobile_list_crashes` rỗng). `flutter test`: 722/722 pass, `flutter analyze` sạch. Chi tiết đầy đủ: `doc/task/done/T65-native-banner-widget-instance-conflict.md`.
