...
| 5 | `lib/src/consent/*` / `applovin_adapter.dart:924-929` | Khoảng hở giữa `_persist()` (await) và `_applyToProviders()` — AppLovin consent thật chỉ áp ở bước sau, AdMob áp ngay | Trong khoảng hở, `canRequestAds` vẫn `true`, retry/refill có thể bắn 1 request AppLovin dùng consent cũ | MAJOR | **FIXED (lần thử thứ 3, 2.4.2).** Lần 1: đóng/mở `_canRequestAds` trực tiếp trong `setConsent()` — vỡ 3 test (source có comment N2 sẵn nói field đó chỉ do UMP flow sở hữu). Lần 2: bắt chước pattern `_pessimisticGateClose`/`_consentIntentEpoch` của `_applyConsentResultOnce` — vỡ NẶNG HƠN: 13 test. Lần 3 (thành công): đọc hết `_applyConsentResultOnce` + 2 file test consent-gate mới hiểu ra `_pessimisticGateClose` giải quyết vấn đề KHÁC (kết quả 1 apply đang QUEUED, chưa biết outcome — "đoán" đóng rồi để `_recoverConsentGate` sửa sau bằng cách hỏi lại device) — không phải vấn đề của finding này (outcome đã BIẾT rồi, chỉ là native AdMob chưa kịp áp xong). Fix bằng field hoàn toàn mới, độc lập: `_consentProviderApplyInFlight`, set/clear trong `finally` quanh ĐÚNG 1 await, chỉ khi hướng tightening (GDPR withdraw/CCPA opt-out) — không đụng `_canRequestAds`/`_pessimisticGateClose`/epoch. Không cần "ai mở lại" vì không persist qua async gap nào. Mutation-verified: `test/consent_provider_apply_in_flight_test.dart` (revert → red, fix → green) + full suite 1342 test pass, kể cả 16 test mà 2 lần thử trước làm vỡ. |
...
4. Finding #5 (khoảng hở consent AdMob/AppLovin) — **ĐÃ FIX ở lần thử thứ 3** (2.4.2), sau khi
   2 lần đầu (2 cách tiếp cận khác nhau, cả hai dựa nhầm vào cơ chế `_pessimisticGateClose`)
   đều revert vì vỡ test (3 rồi 13). Fix cuối dùng 1 field hoàn toàn độc lập
   (`_consentProviderApplyInFlight`), không đụng cơ chế debt/epoch có sẵn.
5. Finding #1, #2 — user chọn để sau, không chặn release đầu.

Suite sau các fix trên: **1.342 test pass** (1.336 baseline + 6 test mới cho #3/#4/#5/#6),
