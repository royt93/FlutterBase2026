...
|---|---|---|
| 2.4.1–current | See `CHANGELOG.md`'s top entry for the current version | Not itemized here — this table stopped being kept in lockstep with releases after 2.4.0. See `CHANGELOG.md` for the real per-version history (round-24 through round-32 audits: consent-apply/TCF-timeout BLOCKER fixes, `AdBootstrapOptions.initTimeout`, AppLovin banner/MREC/native revenue-event wiring, and more). |
| 2.4.0 | Stable (2026-08-26) | Round-23 audit: impression accounting now keys off whether the ad reached the screen (`AdSlot.displayConfirmed`) rather than how the show ended, so a rewarded/rewarded-interstitial the user closed early — and an app-open resolved by the hard cap — count exactly once; VIP timestamps persist as UTC (a device that changed zone or left DST no longer loses grants); the VIP grace nudge is clamped to half the granted window; CRL revocation matches kids case-insensitively; README/prompt-doc fixes. |
| 2.3.0-2.3.4 | Stable | Consent hardening (audit rounds 13-22, on-device verified): `autoRequestUmpConsent` fails closed on a real UMP error, QA test-device hashes always merged into `RequestConfiguration`, GAID cleared on `destroy()`, banner/MREC/native reload actually schedules a frame when the consent gate reopens, rewarded ads re-loadable more than once per session on AdMob. |
| 2.2.0 | Stable | Doc merge (`MIGRATION.md` folded into `doc/AD_PROMPT_FLUTTER.MD` Appendix D) and pub.dev scoring fixes. |
| 2.1.0 | Stable | 2026-08-19 audit fixes: stale ready App Open/interstitial/rewarded ads past their 4h/1h freshness window are now discarded and reloaded instead of shown; `showAppOpenAdOnResume` no longer bypasses the daily/hourly/session safety cap (splash's `bypassSafety: true` is now the *only* sanctioned bypass); `AdSlot.armLoadWatchdog` cancels a stale prior watchdog before re-arming; AppLovin banner/MREC native-view dispose fix. |
