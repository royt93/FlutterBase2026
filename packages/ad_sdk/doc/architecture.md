...
|---|---|---|
| 2.4.0 | Current stable (2026-08-26) | Round-23 audit: impression accounting now keys off whether the ad reached the screen (`AdSlot.displayConfirmed`) rather than how the show ended, so a rewarded/rewarded-interstitial the user closed early — and an app-open resolved by the hard cap — count exactly once; VIP timestamps persist as UTC (a device that changed zone or left DST no longer loses grants); the VIP grace nudge is clamped to half the granted window; CRL revocation matches kids case-insensitively; README/prompt-doc fixes. |
