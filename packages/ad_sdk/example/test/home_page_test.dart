...
void main() {
  Future<void> pumpHomePage(WidgetTester tester) async {
    // The list has 28 tiles — grow the viewport so they all build without
    // needing a scroll gesture.
    tester.view.physicalSize = const Size(800, 5700);
  ...
  testWidgets('renders a DemoTile for every demo', (tester) async {
    ...
    expect(find.byType(DemoTile), findsNWidgets(28));
  ...
  testWidgets('tapping the T166 tile navigates to CcpaToggleDemoPage',
      (tester) async {
    await pumpHomePage(tester);

    await tester.tap(find.text('CCPA opt-out toggle (T166)'));
    await tester.pumpAndSettle();

    expect(find.byType(CcpaToggleDemoPage), findsOneWidget);
    expect(find.byType(HomePage), findsNothing);
  });

  testWidgets('tapping the T168 tile navigates to CustomOverlayDemoPage',
      (tester) async {
    await pumpHomePage(tester);

    await tester.tap(find.text('Custom overlay blocks App Open (T168)'));
    await tester.pumpAndSettle();

    expect(find.byType(CustomOverlayDemoPage), findsOneWidget);
    expect(find.byType(HomePage), findsNothing);
  });

  testWidgets('tapping the T200 tile navigates to ClearSdkDataDemoPage',
      (tester) async {
    await pumpHomePage(tester);

    await tester.tap(find.text('Scoped SDK data erasure (T200)'));
    await tester.pumpAndSettle();
  ...
  testWidgets('tapping the T219 tile navigates to I18nPresetDemoPage',
      (tester) async {
    await pumpHomePage(tester);

    await tester.tap(find.text('i18n string presets (T219)'));
    await tester.pumpAndSettle();

    expect(find.byType(I18nPresetDemoPage), findsOneWidget);
    expect(find.byType(HomePage), findsNothing);
  });
}
