...
- **M6 — FIXED đầy đủ (AdMob trong v2.9.8, AppLovin trong v2.9.9 follow-up).** AdMob: `cycleEnded` local flag (không dùng so sánh field instance — tránh đúng cái bug em tự gây ra khi lẫn lộn "null vì đã tự resolve cùng cycle" với "null vì cycle khác chiếm") cho Interstitial + Rewarded. AppLovin: kiến trúc dùng 1 listener persistent/loại ad (wire lúc `initialize()`), không phải closure riêng mỗi lần `show()` như AdMob, nên dùng ad-identity tracking (`_interstitialAd`/`_rewardedAd`, `identical(ad, ...)`) thay vì local flag. Lần đầu thử implement đã **phá 14+ test có sẵn** vì `applovin_adapter_test.dart` dùng `_fakeAd()` tạo instance MỚI mỗi lần gọi thay vì tái dùng 1 object xuyên suốt load→show→hide (khác thực tế native SDK) — revert, ghi follow-up. User yêu cầu quay lại làm ngay lượt sau: sửa đúng 14 test đó (thread ad reference thật qua, cũng là cải thiện chất lượng test) thay vì revert lần 2. 2 test mới (`round-29 audit follow-up`) mutation-verify RED→GREEN thật cho chính production fix.
