...
## Round 12 (2026-07-28) — audit toàn diện lại từ đầu, 6 sub-agent song song, mỗi agent độc lập không đọc `doc/audit/*` cũ

Yêu cầu round này giống hệt round 10 (audit lại từ đầu, không kế thừa kết luận cũ), map 1-1 vào 7 tiêu chí gốc của user (provider Android/iOS, online/offline, lifecycle+leak từng loại ad đúng pháp lý, trial 1 ngày, VIP-by-code không backend, consent mọi quốc gia, policy compliance) — trial + VIP gộp chung 1 agent vì cùng là cơ chế "no-backend entitlement". Version SDK đã tự verify: root `pubspec.yaml` và `packages/ad_sdk/pubspec.yaml` đều `1.2.2`, khớp bản mới nhất trên pub.dev (fetch trực tiếp xác nhận "Latest version: 1.2.2").

| # | Tiêu chí | Verdict | Ghi chú |
|---|---------|---------|---------|
| 1 | Provider AdMob/AppLovin, Android + iOS | ✅ Đạt | `AdProviderAdapter` interface, 2 adapter tách biệt hoàn toàn (`admob_adapter.dart` 1261 dòng, `applovin_adapter.dart` 1272 dòng), không cross-import. `AndroidManifest.xml`/`Info.plist` đầy đủ config native cho cả 2 platform (APPLICATION_ID, AD_ID/INTERNET permission, `applovin.sdk.key`; `GADApplicationIdentifier`, `AppLovinSdkKey`, `NSUserTrackingUsageDescription`, `SKAdNetworkItems`). **Chưa verify round này**: `gma_mediation_applovin` (dependency native mediation bắt buộc ở app-level) — agent tự flag chưa kiểm tra, cần verify riêng. |
| 2 | Offline/online | ✅ Đạt | Không hồi quy — vẫn `connection_notifier` event-driven, backoff cap 30 phút, teardown huỷ hết Timer/Subscription, `_retryRefillAds()` bail sớm nếu offline (R10-C vẫn đứng vững). |
| 3 | Lifecycle + không leak (banner/mrec/native/interstitial/rewarded/app-open) | ⚠️ Đạt cốt lõi, 1 finding mới thật | Dedup/dispose/RouteAware đều đúng như round 10 xác nhận. **Finding mới (High, likelihood thấp)**: `AdSafetyConfig.dryRun` (`ad_safety_config.dart:103-105,124`) — khi `true` thì bypass toàn bộ cap thật (session/hourly/daily/throttle), chỉ log. Cả preset `production` (dòng 131) và `debug` (dòng 141-152) đều default `false`, nhưng **không có guard rail nào** chặn nếu host vô tình để `dryRun: true` ở release build — khác với footgun AdMob-test-ID đã có `assert()` riêng. |
| 4 | Trial mode 1 ngày | ✅ Đạt, giới hạn đã biết | `FirstInstallVipGrace`, đúng `Duration(days: 1)` (`ad_config.dart:48-49`), 30s ở debug có chủ đích, chống lùi giờ hệ thống. Android vẫn chỉ dựa Auto Backup (không install-referrer/Play Integrity) — trade-off đã document từ round 10/11, không phải phát hiện mới. |
| 5 | VIP by code, không server/backend | ⚠️ Đạt cốt lõi, 2 finding Low mới | Ed25519 qua `package:cryptography` chuẩn, không leak private key (`vip_mint.dart` bắt buộc `--priv`), không đường forge/tamper payload. 3-layer replay guard (in-memory + SharedPreferences + Keychain) nhưng **Keychain ledger chỉ có ở iOS** (`_redeemed_key_ledger.dart:16-19` no-op trên Android — cùng gốc rễ với giới hạn trial ở trên, không phải bug riêng). 2 finding Low mới: (a) `VipEntry.isActive` (`vip_entry.dart:30-34`) chỉ check `now.isBefore(grantedAt)` — chặn được lùi giờ toàn phần nhưng không có high-water-mark nên lùi giờ "một phần" (nhỏ hơn thời điểm cấp quyền hiện có nhưng vẫn sau khi VIP đã active) có thể kéo dài nhẹ; (b) `maxVipStackDuration` clamp (`vip_manager.dart:384-390`) chỉ áp dụng nhánh `stack:true` — nhánh `stack:false` (1 key đơn) chỉ bị chặn bởi `_maxSeconds` nội bộ (~100 năm), tức về lý thuyết 1 key ký hợp lệ với duration cực lớn vẫn qua được nếu không stack. |
| 6 | Consent mọi quốc gia (UMP + CMP) | ✅ Đạt cốt lõi, 1 finding Low mới | Không hồi quy R10-A/R10-B (đã fix ở Round 11, agent xác nhận lại độc lập). ATT 5 state + 20s timeout + fallback `denied` an toàn. TCF/IAB tự đọc qua key chuẩn, không cần forward thủ công. **Finding mới (Low)**: `consentFootgunWarning` (`ad_manager.dart:177-190,1210-1224`) chặn runtime khi AppLovin CMP disabled và UMP chưa từng gọi — nhưng không có nhánh tương đương cho trường hợp `provider = AdMob` và UMP đơn giản chưa từng được gọi (gap trong chính logic cảnh báo, không phải gap ad-serving thật vì `canRequestAds` vẫn là gate runtime chính). |
| 7 | Tuân thủ policy AdMob/AppLovin/Store | ✅ Đạt, 1 điểm hạ mức từ round trước | `ad_keys.dart` vẫn còn AdMob test-unit-ID trên dead code path (R10-F, đã biết, có `assert()` debug chặn từ Round 11, không hồi quy). **Tự verify độc lập finding IAP-pricing của 1 agent** (agent tự nhận "1 nhánh, chưa verify"): `vip_redeem_screen.dart:1195-1294` — 4 nút mua giá cứng (`$0.5/$1/$2/$3`, dòng 1222/1228) cho tính năng `buyLocked = 'SOON'` (chưa implement), cộng 1 dòng "Restore purchase (coming soon)". Đọc trực tiếp code xác nhận **cả nút mua lẫn nút restore đều `onTap: null`** — UI preview tĩnh, không bấm được, không có luồng thanh toán thật. Hạ từ đề xuất "Medium" xuống **Low/Info** (chỉ là placeholder UX, không phải rủi ro billing/store-review thật vì không tương tác được). |

### Chi tiết finding mới round 12

- **R12-A (High, likelihood thấp)** — `AdSafetyConfig.dryRun` không có guard rail release-mode. Đề xuất fix: thêm `assert(!(dryRun && kReleaseMode), ...)` tương tự pattern đã dùng cho AdMob-test-ID footgun (`splash_screen.dart` R10-F) hoặc runtime `SafeLogger.e` + tự động ép `dryRun=false` khi `kReleaseMode`.
- **R12-B (Low)** — `VipEntry.isActive` không có high-water-mark chống lùi giờ một phần.
- **R12-C (Low)** — `maxVipStackDuration` không áp dụng cho nhánh `stack:false`.
- **R12-D (Low)** — `consentFootgunWarning` không cover case `provider=AdMob` + UMP chưa gọi.
- **R12-E (Low/Info, hạ mức)** — IAP "SOON" pricing UI trong `vip_redeem_screen.dart` — đã xác nhận `onTap: null`, chỉ là preview chưa implement, không phải nút bấm thật.
- **Chưa verify round này** — `gma_mediation_applovin` config app-level (native mediation), cần audit riêng lần sau.

### Xác nhận không hồi quy

Cả 6 agent đều xác nhận độc lập, không biết trước: R10-A/B, R10-C/D/E/F/G, và cả VIP Android-reinstall-replay đã document đúng ở Round 10/11 vẫn đứng vững, không có hồi quy nào. Không finding nào ở round 12 lặp lại y hệt finding cũ — tất cả 5 finding mới (R12-A đến R12-E) là góc nhìn/chi tiết chưa từng được ghi nhận ở round 8/9/10/11.

### Kết luận Round 12

**Không có finding Critical.** 1 finding High nhưng likelihood thấp (yêu cầu host tự ý bật `dryRun` ở release, không phải default, không phải lỗ hổng khai thác từ bên ngoài). 4 finding Low (2 VIP edge-case, 1 consent-warning coverage gap, 1 UI placeholder đã hạ mức từ đề xuất Medium sau khi tự verify). Còn 1 hạng mục chưa verify (`gma_mediation_applovin`) cần audit bổ sung, không phải finding tiêu cực — chỉ là chưa kiểm tra.

**Verdict cuối: CÓ — SDK này đủ chất lượng để dùng production**, ở mức **9.0/10** (giữ nguyên mức Round 9/11, không tăng không giảm — không có Critical/High phạm vi rộng mới, các Medium đã biết từ Round 10 đều đã đóng ở Round 11 và không hồi quy). Việc nên làm trước khi khép round này (không chặn ship hiện tại): (1) ~~thêm guard rail release-mode cho `dryRun`~~ **đã fix cùng round (xem bên dưới)**; (2) verify `gma_mediation_applovin` ở app-level; (3) cân nhắc high-water-mark cho `VipEntry.isActive` nếu muốn siết chặt hơn giả định lùi giờ một phần — (2) và (3) là cải thiện phòng ngừa (defense-in-depth), không phải lỗ hổng đang bị khai thác, để lại cho round sau.

### R12-A đã fix cùng round (2026-07-28)

`AdSafetyConfig.init()` giờ chạy `params` qua `applyDryRunReleaseGuard()` (`ad_safety_config.dart`) — nếu `dryRun=true` và đang chạy release build, tự động ép về `dryRun=false` + log `SafeLogger.e` cảnh báo, thay vì chỉ `assert()` (vốn bị strip ở đúng release mode cần chặn nên vô dụng cho case này). Hàm được tách `@visibleForTesting` với tham số `isRelease` override được (mặc định `kReleaseMode`, vốn luôn `false` dưới `flutter test` nên phải override mới test được nhánh release) — cùng pattern testability đã dùng cho `consentFootgunWarning` (F4) trong `ad_manager.dart`. `init()` cũng nhận thêm named param `@visibleForTesting bool isRelease = kReleaseMode` truyền thẳng xuống guard, nên test có thể verify **thật** đường đi end-to-end qua `init()` (không chỉ test hàm guard đứng riêng) mà không cần override `kReleaseMode` (const, không thể override). 5 test trong `ad_safety_config_test.dart` (nhóm `applyDryRunReleaseGuard (R12-A)`, gồm 1 test `init(isRelease: true)` xác nhận `dryRun` bị ép `false` thật qua toàn bộ đường init), toàn bộ suite `ad_sdk` xanh **662/662** (657 cũ + 5 mới), `flutter analyze` sạch. Self code-review sau khi implement: 8.5/10 → phát hiện 1 gap ("test cuối tạo false confidence", không thật sự chứng minh `init()` có wire guard) → đóng gap bằng cách thêm test seam `isRelease` vào `init()` → **10/10**.

> **Lưu ý (audit round 4, 2026-08-01):** đoạn trên là snapshot ngay sau lần self-review đầu tiên của R12-A — đã lỗi thời sau 3 vòng self-review tiếp theo (log method đổi thành `SafeLogger.critical()`, không còn gọi `SafeLogger.e` trực tiếp; thêm test; sửa bug rò state trong `destroy()`; v.v.). Số liệu cuối cùng chính xác: **672/672** test pass, điểm tự-review cuối **9/10** (không phải 10/10) — xem `doc/feature.md` mục R12-A để biết diễn biến đầy đủ 4 vòng hardening.
