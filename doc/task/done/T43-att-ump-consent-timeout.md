...
- **Files:** `packages/ad_sdk/lib/src/core/att_consent.dart`, `packages/ad_sdk/lib/src/core/ump_consent.dart`, `packages/ad_sdk/test/att_consent_test.dart`, `packages/ad_sdk/example/lib/main.dart` (thêm cờ test `SKIP_UMP`)
...
- [x] Có test xác nhận riêng cho nhánh ATT (`fakeAsync` + `Completer` không bao giờ complete) — nhánh UMP không có test riêng vì `ump_consent.dart` gọi thẳng API tĩnh của `google_mobile_ads` (không có tham số `*Override` để giả lập như `att_consent.dart`); code fix dùng đúng `Future.timeout` — cơ chế ngôn ngữ đã được test kỹ ở nhánh ATT — nên rủi ro còn lại chủ yếu nằm ở logic xung quanh, không phải cơ chế timeout.

## Verify
`cd packages/ad_sdk && flutter test` → 552/552 pass (gồm test timeout mới trong `att_consent_test.dart`). `flutter analyze` ở cả `packages/ad_sdk` và repo root → no issues found.
