...
- **T07 done** (ATT wiring đúng thứ tự splash→ATT→UMP; thêm `assert()`-wrapped `SafeLogger.e()` ngay trước `requestAuthorization()` để cảnh báo sớm nếu thiếu `NSUserTrackingUsageDescription`; không cần test mới — nhánh quan sát được không đổi, `att_consent_test.dart` đã bao phủ đủ). Còn lại P1: T15-T16 (provider), T17 (trial) · P2: T14, T19, T20.
- **✅ P0 milestone build verified trên S24 Ultra (SM-S928B) — cả 2 provider:**
  ...
  - **0 crash, 0 `E/flutter`.**
- **290/290 test SDK xanh**, `flutter analyze` sạch (SDK + host).
- ⚠️ Host `pubspec.yaml` đang **flip path override** sang SDK local — nhớ flip lại trước release.
- **Điểm hiện tại: 8/10** — thiếu: (1đ) T14/T15/T16/T17/T19/T20 còn ở todo/; (1đ) chưa re-audit `doc/audit/audit_claude.md` sau đợt fix T07; (~0.5đ) chưa smoke-test lại trên SM_A507FN kể từ khi thêm T07.
...
| T15 | Ad-unit-id tách theo platform (Android/iOS) | 1 | P1 | MEDIUM | todo |
